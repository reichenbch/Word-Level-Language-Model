...
class RNNModel(nn.Module):
	...
	def __init__(self, rnn_type, ntoken, ninp, nhid, nlayers, dropout=0.5, tie_weights=False):
		...
		"""
			...
			and
			Tying Word Vectors and Word Classifiers: A Loss Framework for language modeling. (Inan 2016)
		"""
		if(tie_weights):
			if(nhid != ninp):
...
class TransformerModel(nn.Module):
	"""
		-We are training nn.TransformerEncoder model on a language modeling task.
		...
		-The nn.TransformerEncoder consists of multiple layers of nn.TransformerEncoderLayer. Along with the input sequence, a square attention mask is 
			required because the self-attention layers in nn.TransformerEncoder are only allowed to attend the earlier positions in the sequence. 
